...
```{r, fig.cap="Raw voltage histogram"}
plot1 <- log_data %>%
...
```{r, fig.cap="standardized voltage histogram"}
plot1 <- log_data %>%
           ggplot(aes(x = voltage)) +
           geom_histogram(na.rm = T, bins = 50) +
           labs(x = "Voltage", y = "Count", title = "Log Dataset") +
...
It turns out that when a row contains any missing values, all four variables of interests are missing. Missing values are found in between 2004-04-30 and 2004-05-25 (27 days) for \texttt{log-dataset} and in between 2004-05-07 and 2004-05-29 (22 days) for \texttt{net dataset}. These two time period almost span the entire experiment. Most of the time there is only one record with missing value for each epoch, which indicates that most of the time only one single node is not working well. In the end, 8270 rows of missing values are dropped from \texttt{log dataset} and 4262 rows are dropped from \texttt{net dataset}.

```{r, fig.cap="Count of NAs for each epoch"}
...
```{r, fig.cap="correlation between abnormal readings and low voltage"}
plot1 <- data_all %>%
...
```{r, fig.cap="Histogram after outlier objection", fig.height=5}
plot1 <- data %>%
           ggplot(aes(x = voltage)) +
           geom_histogram(na.rm = T, bins = 50) +
...
```{r, fig.cap="Large-reading nodes' pattern against time"}
nodeid_top <- data %>%
...
There seems to be a positive linear relationship between Incident PAR and height. This is consistent with our common sense that higher nodes receive more sunlight. Lower height nodes have significantly fewer high values for incident PAR. Moreover, reflected PAR and incident PAR are positively correlated from the analysis in last part.
...
```{r fig.cap="Trend of the four environmental variables against height", fig.width=8, fig.height=5}
temp_plot <- data %>%
...
Since there are too many zeros in \texttt{incident PAR} and \texttt{reflected PAR} that are not going to help with tail analysis, we filter all the zeros in the data and take log for transformation.
...
```{r fig.cap="mean of humidity and temperature during a day and during days", fig.width=6, fig.height=4}
# humidity
...
```{r fig.cap="mean of Incident and Reflected PAR during a day and during days", fig.width=6, fig.height=4}
# hamatop
...
```{r fig.cap="plot simplification", fig.width=6}
# humid
...
```{r, fig.cap="A better visualization for net and log data", fig.height=5, fig.width=5}
library(scales)
all %>%
