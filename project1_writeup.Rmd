...
```{r, fig.cap="different scales for voltage", fig.width=6, fig.height=2}
plot1 <- log_data %>%
...
plot3 <- merge(data, nodeid_top, by = "nodeid") %>%
  mutate(nodeid = as.character(nodeid)) %>%
...
plot4 <- merge(data, nodeid_bottom, by = "nodeid") %>%
  mutate(nodeid = as.character(nodeid)) %>%
...
```{r fig.cap="selected scatter plots", fig.width=8, fig.height=5}
fig1 <- data %>%
...
```{r fig.cap="time series plots for the four environmental variables", fig.width=8, fig.height=8}
temp_plot <- data %>%
...
grid.arrange(temp_plot, humidity_plot, incident_plot, reflected_plot, nrow = 4)
```
...
```{r fig.cap="gmm grouping result", fig.width=10, fig.height=5}
...
```{r fig.cap="pca", fig.width=6, fig.height=4}
# PCA
library(ggfortify)
...
```{r fig.cap="k-means against height and time", fig.width=8, fig.height=5}
...
grid.arrange(height1, height2, hour1, hour2, nrow = 2, ncol = 2)
```
...
plot2 <- data %>% 
  ...
  labs(x = "mol/m2/s", y = "counts", title = "Reflected PAR")
...
```{r fig.cap="mean of four enviromental variables during a day and across different days", fig.width=8, fig.height=8}
# humidity
