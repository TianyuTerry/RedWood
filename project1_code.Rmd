...
Right now all min/max in the summary statistics for 4 variables are properly solved/explained.
...
```
```{r}
min(net_data_loc$result_time)
max(net_data_loc$result_time)
```

```{r}
fig1 <- net_data_loc %>%
  filter(date(result_time) < '2004-05-15') %>%
...
```{r}
fig3 <- net_data_loc %>%
  filter(date(result_time) < '2004-06-03') %>%
